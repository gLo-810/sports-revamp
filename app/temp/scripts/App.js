...
var SaveInput = function () {
  ...
  function SaveInput() {
    ...
    this.mainText = (0, _jquery2.default)('.main-text');
    this.names = [];
    this.numbers;
    this.events();
  }
  ...
  _createClass(SaveInput, [{
    ...
    key: 'saveNameIndex',
    value: function saveNameIndex() {
      var namesResult = this.names = this.mainText.val().split('\n');

      //creates a numbers array that is the same length as the names array, for indexing.
      var numsResult = this.numbers = Array.from({ length: this.names.length }).map(function (_, i) {
        return i;
      });

      console.log(namesResult);
      console.log(numsResult);
      return { namesResult: namesResult, numsResult: numsResult };
    }
  }]);
